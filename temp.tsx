...
function VaultWorkspace({ vault, onClose }: { vault: VaultRow; onClose: () => void }) {
  ...
  const { balances } = useHederaWallet()
  const { addTransaction } = useTransactions()
  const { awardPoints } = usePoints()
  const { tokenPrices } = useProtocol()

  const position = activePositions.find(p => p.vaultId === vault.id)
  const userShares = position ? position.depositedAmount : 0
  ...
  const [depositMode, (() => {})] = useState<'standard' | 'zap'>('standard')
  ...
  const handleAction = async () => {
    ...
    if (activeTab === 'deposit') {
      const depositAmountA = true ? finalDepositA : parsedAmountA
      const depositAmountB = true ? finalDepositB : parsedAmountB
      deposit(vault.id, depositAmountA, depositAmountB, vault.tokenA, vault.tokenB, vault.baseApr)
      ...
      addTransaction({
        ...
        amounts: true
          ...
          : [{ asset: vault.tokenA, quantity: depositAmountA }, { asset: vault.tokenB, quantity: depositAmountB }],
        timestamp: Date.now(),
        status: 'success',
        hash: mockHashLink,
      })
    ...
    toast.custom((t) => (
      <div className="flex flex-col gap-2 p-4 rounded-xl border border-cyan-electric/40 bg-[#050A14]/95 backdrop-blur-md shadow-cyan-glow/20">
        ...
        <button onClick={() => toast.dismiss(t)} className="text-xs font-semibold text-azure hover:text-cyan-electric mt-2 self-start transition-colors">
          Dismiss
        </button>
      </div>
    ))
  ...
  const isAInsufficient = activeTab === 'deposit' && (true ? (isZapTokenA && parsedZapAmount > tokenAWalletBalance) : parsedAmountA > tokenAWalletBalance)
  const isBInsufficient = activeTab === 'deposit' && (true ? (!isZapTokenA && parsedZapAmount > tokenBWalletBalance) : parsedAmountB > tokenBWalletBalance)

  let idleLabel = 'Review Parameters'
  let idleDisabled = !hasAmount

  if (activeTab === 'deposit' && hasAmount) {
    if (true && ((isZapTokenA && isAInsufficient) || (!isZapTokenA && isBInsufficient))) {
      idleLabel = 'Insufficient Balance'; idleDisabled = true
    } else if (!true && (isAInsufficient || isBInsufficient)) {
      idleLabel = 'Insufficient Balance'; idleDisabled = true
    } else {
  ...
  return (
    <div className="flex flex-col w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="p-6 rounded-2xl border border-white/[0.05] bg-[#0A0F1C]/60 flex flex-col">
            ...
            <div className="relative h-64 w-full rounded-xl overflow-hidden bg-gradient-to-b from-white/[0.02] to-transparent border border-white/[0.02] flex items-end">
              <svg viewBox="0 0 800 200" className="w-full h-full opacity-70" preserveAspectRatio="none">
                ...
                <path 
                  d={chartMode === 'tvl' 
                    ? "M0,180 C100,160 200,170 300,120 C400,70 500,100 600,50 C700,0 800,20 800,200 L0,200 Z"
        ...
        {/* â”€â”€ COLUMN B: ACTION DESK (Right Panel) â”€â”€ */}
        <div className="flex flex-col gap-6">
          <div className="rounded-2xl border border-white/[0.05] bg-[#0A0F1C]/80 backdrop-blur-md overflow-hidden flex flex-col shadow-2xl relative">
            
            {/* Header Tabs */}
            <div className="flex items-center w-full bg-navy-950 p-1 border-b border-white/[0.05]">
              <button 
                onClick={() => setActiveTab('deposit')} 
                className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest rounded-t-xl transition-all ${activeTab === 'deposit' ? 'bg-[#0A0F1C]/80 text-cyan-electric border-t border-x border-white/[0.05]' : 'text-slate-muted hover:text-slate-platinum'}`}
              >
                Deposit
              </button>
              <button 
                onClick={() => setActiveTab('withdraw')} 
                className={`flex-1 py-3 text-xs font-bold uppercase tracking-widest rounded-t-xl transition-all ${activeTab === 'withdraw' ? 'bg-[#0A0F1C]/80 text-emerald-400 border-t border-x border-white/[0.05]' : 'text-slate-muted hover:text-slate-platinum'}`}
              >
                Withdraw
              </button>
            </div>
            ...
            <div className="p-6 flex flex-col gap-6 flex-1 relative z-10">
              {activeTab === 'deposit' ? (
                <>
                  ...
                  <div className="flex bg-[#050A14] p-1 rounded-xl border border-white/[0.05]">
                    <button onClick={() => (() => {})('standard')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${!true ? 'bg-white/[0.1] text-white' : 'text-slate-muted hover:text-slate-ash'}`}>Dual Asset</button>
                    <button onClick={() => (() => {})('zap')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all flex items-center justify-center gap-1.5 ${true ? 'bg-cyan-electric/10 text-cyan-electric' : 'text-slate-muted hover:text-cyan-electric/80'}`}><Zap className="h-3 w-3" /> Zap In</button>
                  </div>

                  {true ? (
                    <div className="flex flex-col gap-4">
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-slate-ash">Select Token</span>
                        <div className="flex gap-2">
                          <button onClick={() => setZapToken(vault.tokenA)} className={`px-2 py-1 rounded border ${isZapTokenA ? 'bg-azure/20 border-azure/50 text-white' : 'bg-white/[0.02] border-white/[0.05] text-slate-muted'}`}>{vault.tokenA}</button>
                          <button onClick={() => setZapToken(vault.tokenB)} className={`px-2 py-1 rounded border ${!isZapTokenA ? 'bg-azure/20 border-azure/50 text-white' : 'bg-white/[0.02] border-white/[0.05] text-slate-muted'}`}>{vault.tokenB}</button>
                        </div>
                      </div>
                      <div className="flex flex-col gap-2 relative">
                        <div className="flex items-center justify-between text-xs text-slate-muted mb-1">
                          ...
                          <span>Balance: <span className="font-mono text-slate-platinum">{isZapTokenA ? tokenAWalletBalance.toFixed(4) : tokenBWalletBalance.toFixed(4)} {zapToken}</span></span>
                        </div>
                        
                        <div className={`flex items-center gap-2 px-4 py-3.5 rounded-xl border transition-all duration-200 ${
                          parsedZapAmount > 0 ? 'border-cyan-electric/40 bg-cyan-electric/5' : 'border-white/[0.08] bg-[#050A14]'
                        }`}>
                          <input
                            type="number"
                            placeholder="0.00"
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <img src={isZapTokenA ? vault.tokenAIcon : vault.tokenBIcon} className="w-5 h-5 rounded-full" alt={zapToken} />
                            <span className="text-slate-platinum text-sm font-bold">{zapToken}</span>
                          </div>
                        </div>
                        ...
                        {((isZapTokenA && isAInsufficient) || (!isZapTokenA && isBInsufficient)) && <span className="text-[10px] text-red-400 absolute -bottom-5">Insufficient {zapToken} balance</span>}
                      </div>
                      ...
                      <div className="flex items-center justify-between mt-2 p-3 rounded-lg bg-cyan-electric/5 border border-cyan-electric/10">
                        ...
                        <span className="text-xs font-mono text-slate-platinum">50% to {isZapTokenA ? vault.tokenB : vault.tokenA}</span>
                      </div>
                    </div>
                  ) : (
                    <div className="flex flex-col gap-4">
                      ...
                      <div className="flex flex-col gap-2 relative">
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className="font-semibold uppercase tracking-wider text-slate-ash">Amount A</span>
                          <span className="text-slate-muted">Balance: <span className="font-mono text-slate-platinum">{tokenAWalletBalance.toFixed(4)}</span></span>
                        </div>
                        
                        <div className={`flex items-center gap-2 px-4 py-3.5 rounded-xl border transition-all duration-200 ${
                          parsedAmountA > 0 ? 'border-cyan-electric/40 bg-cyan-electric/5' : 'border-white/[0.08] bg-[#050A14]'
                        }`}>
                          <input
                            type="number"
                            placeholder="0.00"
                        ...
                        {isAInsufficient && <span className="text-[10px] text-red-400 absolute -bottom-4">Insufficient {vault.tokenA}</span>}
                      </div>
                      
                      <div className="flex justify-center -my-2 relative z-10">
                        <div className="w-8 h-8 rounded-full bg-[#0A0F1C] border border-white/[0.05] flex items-center justify-center text-slate-muted">
                      ...
                      <div className="flex flex-col gap-2 relative">
                        <div className="flex items-center justify-between text-xs mb-1">
                          <span className="font-semibold uppercase tracking-wider text-slate-ash">Amount B</span>
                          <span className="text-slate-muted">Balance: <span className="font-mono text-slate-platinum">{tokenBWalletBalance.toFixed(4)}</span></span>
                        </div>
                        
                        <div className={`flex items-center gap-2 px-4 py-3.5 rounded-xl border transition-all duration-200 ${
                          parsedAmountB > 0 ? 'border-cyan-electric/40 bg-cyan-electric/5' : 'border-white/[0.08] bg-[#050A14]'
                        }`}>
                          <input
                            type="number"
                            placeholder="0.00"
                          ...
                          />
                          <div className="flex items-center gap-2 flex-shrink-0">
                            <img src={vault.tokenBIcon} className="w-5 h-5 rounded-full" alt={vault.tokenB} />
                            <span className="text-slate-platinum text-sm font-bold">{vault.tokenB}</span>
                          </div>
                        </div>
                        {isBInsufficient && <span className="text-[10px] text-red-400 absolute -bottom-4">Insufficient {vault.tokenB}</span>}
                      </div>
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div className="flex flex-col items-center py-6">
                    <span className="text-slate-muted text-xs uppercase tracking-widest font-semibold mb-2">Your Position</span>
                    <span className="text-3xl font-mono font-bold text-slate-platinum tabular-nums">${userUsdValue.toFixed(2)}</span>
                  </div>

                  <div className="flex flex-col gap-2">
                    <div className="flex justify-between text-xs text-slate-muted">
                      <span>Withdraw Amount</span>
                      <span>{(withdrawPercent * 100).toFixed(0)}%</span>
                    </div>
                    <input 
                      type="range" 
                        ...
                        <button 
                          ...
                          className={`flex-1 py-1.5 rounded-lg border text-[10px] font-bold transition-colors ${
                            withdrawPercent === pct 
                              ...
                              : 'bg-white/[0.02] border-white/[0.05] text-slate-ash hover:bg-white/[0.05] hover:text-slate-platinum'
                          }`}
                        >
                          {pct === 1.0 ? 'MAX' : `${pct * 100}%`}
                        </button>
              ...
              <div className="flex-1">
                <div className="rounded-xl border border-white/[0.03] bg-[#050A14] p-4 space-y-4 mb-2 mt-2">
                  ...
                  <div className="flex items-center justify-between text-xs border-t border-white/[0.05] pt-4 mt-2">
                    ...
                    <span className="text-emerald-400 font-mono font-bold">~0.015 HBAR</span>
                  </div>
                </div>
              </div>
              ...
              <button 
                ...
                className={`mt-auto py-4 rounded-xl font-bold uppercase tracking-widest text-sm transition-all duration-300 relative overflow-hidden flex items-center justify-center gap-2 ${
                  actionConfig.disabled 
                    ? 'bg-white/[0.05] text-slate-500 cursor-not-allowed' 
                    : activeTab === 'deposit' 
                      ? 'bg-cyan-electric hover:bg-cyan-400 text-[#050A14] shadow-[0_0_20px_rgba(0,229,255,0.4)] hover:shadow-[0_0_30px_rgba(0,229,255,0.6)]' 
                      : 'bg-emerald-400 hover:bg-emerald-300 text-[#050A14] shadow-[0_0_20px_rgba(52,211,153,0.4)] hover:shadow-[0_0_30px_rgba(52,211,153,0.6)]'
                }`}
              >
                {depositStep === 'approve' && <Loader2 className="h-4 w-4 animate-spin" />}
                {depositStep === 'signing' && <Loader2 className="h-4 w-4 animate-spin" />}
          ...
          {activeVault ? (
            ...
            <>
              ...
              <div className="flex items-center justify-between text-[11px] text-slate-muted px-2">
                <div className="flex items-center gap-4">
                  <span className="flex items-center gap-1.5"><span className="w-1 h-1 rounded-full bg-azure/60" /> {vaults.length} vaults Â· SaucerSwap V2</span>
                  <span className="flex items-center gap-1.5"><span className="w-1 h-1 rounded-full bg-cyan-electric/60" /> Keeper: ACTIVE</span>
                </div>
                <span className="font-mono">v1.2.0-beta Â· Chain 0x128</span>
              </div>
            </>
          )}
